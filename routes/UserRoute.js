  ...
  try {
    ...
    if (oldUser) {
      if (oldUser.firstName == " ") {
        ...
        mailer.sendConfirmationEmail(email, oldUser.code);
...
userRouter.post("/registerrestaurants", async (req, res) => {
  ...
  try {
    ...
    const temp_restaurants = new Restaurants({
      logo: logo,
...
userRouter.post("/resetpassword:code", async (req, res) => {
  const { email, password } = req.body;
  try {
    // Check if the email is valid
    if (!password) {
